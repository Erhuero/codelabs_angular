...
export class RoomService {
  ...
  getRooms(): Observable<Room[]> {
    ...
    return this.http.get<Room[]>(this.apiUrl).pipe(catchError(this.handleError));
  }
  ...
  modifyRoom(room: Room): Observable<Room> {
    ...
    return this.http.put<Room>(`${this.apiUrl}/${room.id}`, room).pipe(catchError(this.handleError));
  }
  ...
  getRoomById(id: number): Observable<Room> {
    ...
    return this.http.get<Room>(`${this.apiUrl}/${id}`).pipe(catchError(this.handleError));
  }
  ...
  deleteRoom(id: number): Observable<{ message: string }> {
    ...
    //const headers = new HttpHeaders({
      //Authorization: `Bearer ${token}`});
    return this.http.delete<{ message: string }>(`${this.apiUrl}/${id}`).pipe(map(() => ({ message: 'Salle supprimée avec succès.' })),catchError(this.handleError)
      );
  }
}
